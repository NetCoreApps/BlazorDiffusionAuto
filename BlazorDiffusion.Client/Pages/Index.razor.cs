...
public partial class Index : AppAuthComponentBase, IDisposable
{
    ...
    [Inject]
    ILogger<Index> Log { get; set; } = default!;

    [Inject]
    IIntersectionObserverService ObserverService { get; set; } = default!;
    ...
    async Task updateAsync()
    {
        ...
        {
            SetTitle((SelectedUser?.Handle != null ? "@" + SelectedUser?.Handle : "User") + " " +
                     (show ?? "creations"));
        }
    ...
    void setResults(IEnumerable<Artifact> results)
    {
        ...
        GalleryResults = X.Apply(GalleryResults.Clone(),
            x => x.Artifacts = UserState.GetArtifactsForGrid(this.results));
        StateHasChanged();
    }
